...
public class CannyEdgeDetector {
   ...
   * PImage.TYPE_INT_ARGB in which edge pixels are white and all other
   * pixels are black.
   * 
  ...
  private void writeEdges(int pixels[]) {
    ...
    //in any other format other than an INT_ARGB type PImage.
    //This may be easily remedied by providing alternative accessors.
    if (edgesImage == null) {
      edgesImage = new PImage(width, height);
    }
    edgesImage.loadPixels( );
    for( int pdx = 0; pdx < picsize; ++pdx )
      edgesImage.pixels[pdx] = pixels[pdx];
    edgesImage.updatePixels( );
  }
