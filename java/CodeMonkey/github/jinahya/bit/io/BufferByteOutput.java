...
public class BufferByteOutput< T extends ByteBuffer > extends AbstractByteOutput< T > {
	...
	public static BufferByteOutput< ByteBuffer > of( final int capacity, final WritableByteChannel channel ) {
		...
		return new BufferByteOutput< ByteBuffer >( null ) {
			...
			public void write( final int value ) throws IOException {
				...
				if( this.target == null ) {
					this.target = ByteBuffer.allocate( capacity ); // position: zero, limit: capacity
				}
				super.write( value );
				if( !this.target.hasRemaining( ) ) { // no space to put
					this.target.flip( ); // limit -> position, position -> zero
					do {
						channel.write( this.target );
					} while( this.target.position( ) == 0 );
					this.target.compact( );
				}
			}
		};
	 ...
	 *
	 * @param target the {@code ByteBuffer} to which bytes are written; {@code null}
	 ...
	 * invokes {@link ByteBuffer#put(byte)}, on what {@link #getTarget()} gives,
	 * with given {@code value}. Override this method if the {@link #target} is
