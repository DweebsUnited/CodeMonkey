...
public class ManyArrows extends ProjectBase {
	...
	public void setup( ) {
		...
		for( int pdx = 0; pdx < this.nPs; ++pdx ) {
			...
			float[ ] d = { this.rng.nextFloat( ), this.rng.nextFloat( ), this.rng.nextFloat( ), this.rng.nextFloat( ),
					this.rng.nextFloat( ) };
			float[ ] v = { this.rng.nextFloat( ), this.rng.nextFloat( ), this.rng.nextFloat( ), this.rng.nextFloat( ),
					this.rng.nextFloat( ) };
			nD vv = new nD( v );
			vv.normalize( );
	...
	public void draw( ) {
		...
		for( int pdx = 0; pdx < this.nPs; ++pdx ) {
			...
			if( !inBounds ) {
				...
				p.set( new float[ ] { this.rng.nextFloat( ), this.rng.nextFloat( ), this.rng.nextFloat( ),
						this.rng.nextFloat( ), this.rng.nextFloat( ) } );
				v.set( new float[ ] { 2 * this.rng.nextFloat( ) - 1, 2 * this.rng.nextFloat( ) - 1,
						2 * this.rng.nextFloat( ) - 1, 2 * this.rng.nextFloat( ) - 1, 2 * this.rng.nextFloat( ) - 1 } );
				v.normalize( );
				v.mult( this.vMag );
