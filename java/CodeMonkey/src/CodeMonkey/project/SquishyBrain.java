...
public class SquishyBrain extends ProjectBase {
	...
	private final float DELY_SQ_LEN_CUTOFF = PApplet.max( this.sWidth, this.sHeigh ) / 2;
	...
	public void setup( ) {
		...
		// Poisson sample a bunch in the middle, plus the four corners fixed connected
		// to the extremis
		...
		this.saveJSONArray( pts, ProjectBase.dataDir + "SquishyBrain.json" );


		// Call TombstoneTriangulator
		try {

			Process p = Runtime.getRuntime( ).exec( new String[ ] { ProjectBase.CM + "TombstoneTriangulator/TombstoneTriangulator",
					"z", ProjectBase.dataDir + "SquishyBrain.json", ProjectBase.dataDir + "SBTriang.json" } );
		...
		JSONObject triangulation = this.loadJSONObject( ProjectBase.dataDir + "SBTriang.json" );
