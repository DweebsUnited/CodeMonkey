...
public class DopeyEevee extends PApplet {
	...
	private static String dataDir = DopeyEevee.CM + "data/";
	...
	private PipelineTransform makeTransform( Random rng ) {
		...
		switch( rng.nextInt( 5 ) ) {
			...
			case 0:
				a = rng.nextInt( DopeyEevee.N_PIPE );
				b = rng.nextInt( DopeyEevee.N_PIPE );
				c = rng.nextInt( DopeyEevee.N_PIPE );
				while( b == a )
					b = rng.nextInt( DopeyEevee.N_PIPE );

				t = new PipelineTransform( new CodeMonkey.transform.dualCoordinate.DCTAdd( ), a, b );
			...
			case 1:
				...
				t = new PipelineTransform( new CodeMonkey.transform.dualCoordinate.DCTSub( ), a, b );
			...
			case 2:
				...
				t = new PipelineTransform( new CodeMonkey.transform.coordinate.CTNoiseRotate( this ), a );
			...
			case 3:
				...
				t = new PipelineTransform( new DCTMidpointDisplace( rng ), a, b );
			...
			case 4:
				...
				t = new PipelineTransform( new CTExponential( 1, p, 0 ), a );
	...
	private void makeDraw( ) {
		...
		this.pipeline = new PipelineTransform[ DopeyEevee.N_TRNS ];
		for( int tdx = 0; tdx < DopeyEevee.N_TRNS; ++tdx )
		...
		for( int xdx = 0; xdx < this.pixelWidth; ++xdx ) {
			...
			for( int ydx = 0; ydx < this.pixelHeight; ++ydx ) {
				...
				// };
				// ( x, y, n ) * 3
				PVector[ ] pipelineState = { this.ohoneToNegpos.map( new PVector( x, y, n ) ),
				...
				for( int pdx = 0; pdx < DopeyEevee.N_PIPE; ++pdx )
		...
		// Set each buffer up
		// WTF, for what?!
		for( int bdx = 0; bdx < DopeyEevee.N_PIPE; ++bdx ) {
			...
			for( int xdx = 0; xdx < this.pixelWidth; ++xdx ) {
				...
				for( int ydx = 0; ydx < this.pixelHeight; ++ydx ) {
					...
					this.canvas[ bdx ].pixels[ xdx + ydx * this.pixelWidth ] = this.colorTrans.map( this,
							this.buffer[ bdx ][ xdx + ydx * this.pixelWidth ] );
		...
		this.canvas[ DopeyEevee.N_PIPE ].beginDraw( );
		this.canvas[ DopeyEevee.N_PIPE ].loadPixels( );

		float[ ] w = new float[ DopeyEevee.N_PIPE ];
		float wm = 0;
		for( int pdx = 0; pdx < DopeyEevee.N_PIPE; ++pdx ) {
		...
		for( int xdx = 0; xdx < this.pixelWidth; ++xdx ) {
			for( int ydx = 0; ydx < this.pixelHeight; ++ydx ) {
				...
				this.canvas[ DopeyEevee.N_PIPE ].pixels[ xdx + ydx * this.pixelWidth ] = this.colorTrans.map( this,
						new PVector( r, g, b ) );

				int red = ( this.canvas[ DopeyEevee.N_PIPE ].pixels[ xdx + ydx * this.pixelWidth ] >> 0 ) & 0x000000FF;
				int green = ( this.canvas[ DopeyEevee.N_PIPE ].pixels[ xdx + ydx * this.pixelWidth ] >> 8 ) & 0x000000FF;
				int blue = ( this.canvas[ DopeyEevee.N_PIPE ].pixels[ xdx + ydx * this.pixelWidth ] >> 16 ) & 0x000000FF;
		...
		for( int xdx = 0; xdx < this.pixelWidth; ++xdx ) {
			for( int ydx = 0; ydx < this.pixelHeight; ++ydx ) {
				...
				int p = this.canvas[ DopeyEevee.N_PIPE ].pixels[ xdx + ydx * this.pixelWidth ];

				int red = ( p >> 0 ) & 0x000000FF;
				int green = ( p >> 8 ) & 0x000000FF;
				int blue = ( p >> 16 ) & 0x000000FF;

				red = PApplet.round( rTrans.map( red ) ) & 0xFF;
				green = PApplet.round( gTrans.map( green ) ) & 0xFF;
				blue = PApplet.round( bTrans.map( blue ) ) & 0xFF;

				p = red | ( green << 8 ) | ( blue << 16 ) | ( p & 0xFF000000 );

				this.canvas[ DopeyEevee.N_PIPE ].pixels[ xdx + ydx * this.pixelWidth ] = p;

			}
		}

		this.canvas[ DopeyEevee.N_PIPE ].updatePixels( );
		this.canvas[ DopeyEevee.N_PIPE ].endDraw( );
	...
	public void setup( ) {
		...
		this.canvas = new PGraphics[ DopeyEevee.N_PIPE + 1 ];
		for( int pdx = 0; pdx < DopeyEevee.N_PIPE + 1; ++pdx )
			this.canvas[ pdx ] = this.createGraphics( this.pixelWidth, this.pixelHeight );

		this.buffer = new PVector[ DopeyEevee.N_PIPE ][ ];
		for( int bdx = 0; bdx < DopeyEevee.N_PIPE; ++bdx ) {
	...
	public void keyPressed( ) {
		...
		if( this.key == ' ' )
			...
			this.save( DopeyEevee.dataDir + "DopeyEevee.png" );
		else if( this.key == 'a' )
			this.canvas[ 0 ].save( DopeyEevee.dataDir + "DopeyEevee.png" );
		else if( this.key == 'r' )
			this.canvas[ 1 ].save( DopeyEevee.dataDir + "DopeyEevee.png" );
		else if( this.key == 'z' )
			this.canvas[ 2 ].save( DopeyEevee.dataDir + "DopeyEevee.png" );
		else if( this.key == 'x' )
			this.canvas[ 3 ].save( DopeyEevee.dataDir + "DopeyEevee.png" );
