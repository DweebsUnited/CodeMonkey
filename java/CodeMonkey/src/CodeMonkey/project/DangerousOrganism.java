...
public class DangerousOrganism extends ProjectBase {
	...
	public void setup( ) {
		...
		this.mSz = PApplet.min( this.cWidth, this.cHeigh );
		...
		for( int ydx = 0; ydx < this.N_GRID; ++ydx ) {
			for( int xdx = 0; xdx < this.N_GRID; ++xdx ) {
				...
				this.g[ ydx ][ xdx ] = new PVector( this.xMar + this.gPxCellHlf + this.gPxCell * xdx,
	...
	public void draw( ) {
		...
		for( int ydx = 0; ydx < this.N_GRID; ++ydx ) {
			for( int xdx = 0; xdx < this.N_GRID; ++xdx ) {
				...
				PVector TR = new PVector( this.gCornerLen, 0 ).rotate( 1 * PConstants.PI / 4 ).add( this.g[ ydx ][ xdx ] );
				PVector TL = new PVector( this.gCornerLen, 0 ).rotate( 3 * PConstants.PI / 4 ).add( this.g[ ydx ][ xdx ] );
				PVector BL = new PVector( this.gCornerLen, 0 ).rotate( 5 * PConstants.PI / 4 ).add( this.g[ ydx ][ xdx ] );
				PVector BR = new PVector( this.gCornerLen, 0 ).rotate( 7 * PConstants.PI / 4 ).add( this.g[ ydx ][ xdx ] );
