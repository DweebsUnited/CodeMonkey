...
public class FrozenPlanetoid extends ProjectBase {
	...
	private float[ ] grad = { 0.05f, 0.20f, 0.05f, 0.20f, 0, 0.20f, 0.05f, 0.20f, 0.05f };
			...
			for( int dx = 0; dx < this.cWidth; ++dx ) {
				...
				for( int gy = -1; gy < 2; ++gy ) {
					...
					for( int gx = -1; gx < 2; ++gx ) {
						...
						// g *= ( 1 + gx * this.gDx[ pdx + gx + gy * this.cWidth ] + gy * this.gDy[ pdx
						// + gx + gy * this.cWidth ] );
						// float fx = this.toNegPos.map( this.noise( ( dx + gx ) * 0.005f, ( dy + gy ) *
						// 0.005f, 0 + this.frameCount * 0.005f ) );
						// float fy = this.toNegPos.map( this.noise( ( dx + gx ) * 0.005f, ( dy + gy ) *
						// 0.005f, 5 + this.frameCount * 0.005f ) );
	...
	public void setup( ) {
		...
		for( int dy = 0; dy < this.cHeigh; ++dy ) {
			for( int dx = 0; dx < this.cWidth; ++dx ) {
				...
				this.AField[ pdx ] = 1;
				this.BField[ pdx ] = ( Math.abs( dx - this.cWidth / 2 ) < 10 && Math.abs( dy - this.cHeigh / 2 ) < 10 )
						? 1
						: 0;
		...
		System.out.println( String.format( "A: %f, B: %f, Da: %f, Db: %f, f: %f, k: %f", AAvg / this.nPx,
				BAvg / this.nPx, DaAvg / this.nPx, DbAvg / this.nPx, fAvg / this.nPx, kAvg / this.nPx ) );
