...
public class BulbousFlagon extends ProjectBase {
  ...
  private void recurse( ) {
    ...
    for( int dx = -1; dx <= 1; ++dx ) {
      for( int dy = -1; dy <= 1; ++dy ) {
        ...
        this.canvas.pixels[ nx + ny * this.cWidth ] = 0xFF000000 | ( ( ro & 0xFF ) << 0x00 ) | ( ( go & 0xFF ) << 0x08 ) | ( ( bo & 0xFF ) << 0x10 );
  ...
  public void setup( ) {
    ...
    this.canvas.pixels[ this.cWidth / 2 + this.cHeigh / 2 * this.cWidth ] = this.color( this.rng.nextInt( 256 ), this.rng.nextInt( 256 ), this.rng.nextInt( 256 ) );
