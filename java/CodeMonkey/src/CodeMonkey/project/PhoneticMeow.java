  ...
  public void draw( ) {
    ...
    for( int gdy = 1; gdy < this.GRID_Y - 1; ++gdy ) {
      ...
      for( int gdx = 1; gdx < this.GRID_X - 1; ++gdx ) {
        ...
        F.add( Ft );
        Spring.spring(
            ( k + this.gridK.get( gdy - 1 ).get( gdx ) ) / 2,
            p.c,
            this.gridPoints.get( gdy - 1 ).get( gdx ).c,
            Ft,
            null );
        F.add( Ft );
        Spring.spring(
    ...
    // Mass update
    float cx = ( this.GRID_X - 3 ) * ( (float)Math.sin( this.frameCount / this.fx * 2 * PI ) + 1 ) / 2f;
    float cy = ( this.GRID_Y - 3 ) * ( (float)Math.sin( this.frameCount / this.fy * 2 * PI ) + 1 ) / 2f;
