...
public class Plane implements Intersectable, CoordinateTransform {
	...
	public Plane( PVector norm, PVector o ) {
		...
		float angX = Math.min( PVector.angleBetween( norm, new PVector( 1, 0, 0 ) ),
				PVector.angleBetween( norm, new PVector( -1, 0, 0 ) ) );
		float angY = Math.min( PVector.angleBetween( norm, new PVector( 0, 1, 0 ) ),
				PVector.angleBetween( norm, new PVector( 0, -1, 0 ) ) );
		float angZ = Math.min( PVector.angleBetween( norm, new PVector( 0, 0, 1 ) ),
		...
		// );
		// System.out.println( String.format( "O: %f, %f, %f", this.orig.x, this.orig.y,
		// this.orig.z ) );
		// System.out.println( String.format( "U: %f, %f, %f", this.up.x, this.up.y,
		// this.up.z ) );
		// System.out.println( String.format( "R: %f, %f, %f", this.right.x,
