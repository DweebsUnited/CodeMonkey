...
public class PoissonSampler3D {
  ...
  public PoissonSampler3D( float width, float height, float depth, float mDist, int nCount ) {
    ...
    this.grid = new Grid3D<PVector>(
        (int) Math.ceil( width  / this.cellSize ),
        (int) Math.ceil( height / this.cellSize ),
        (int) Math.ceil( depth  / this.cellSize ) );
    ...
    PVector point = new PVector(
        PoissonSampler3D.rng.nextFloat( ) * this.width,
        PoissonSampler3D.rng.nextFloat( ) * this.height,
        PoissonSampler3D.rng.nextFloat( ) * this.depth );
