...
// This file adapted from SigHack, Manohar Vanga
// Check out the original at: https://sighack.com/post/poisson-disk-sampling-bridsons-algorithm
// I have made minor modifications to accept custom canvas sizes, and support retina displays
...
ArrayList<PVector> poissonDiskSampling(float radius, int cwidth, int cheight, int k) {
  int N = 2;
  ...
  /* Initial point p0 */
  PVector p0 = new PVector(random(cwidth), random(cheight));
  PVector[][] grid;
  float cellsize = floor(radius/sqrt(N));

  /* Figure out no. of cells in the grid for our canvas */
  int ncells_width = ceil(cwidth/cellsize) + 1;
  int ncells_height = ceil(cheight/cellsize) + 1;
